...
import { Check, ChevronLeft, Crown, Hourglass, Trophy, X } from "lucide-react-native";
import Animated, {
	Easing,
	useAnimatedStyle,
	useSharedValue,
	...
	return (
		<Box className="mx-3 mt-3">
			...
			{isLoading ? (
				...
				<Box className="mt-6 rounded-3xl border border-border bg-card px-5 py-8 items-center">
					<Text className="text-base font-semibold">
						{t("contest.ranking.emptyTitle", "Reyting hali yo'q")}
					</Text>
...
export default function ContestScreen() {
	...
	const isContestLoading =
		contestsQuery.isLoading || (contestsQuery.isFetching && contests.length === 0);
	const scopeLabel =
		contestScope === "global"
			? t("contest.scope.global", "Global")
			: t("contest.scope.academy", "Academy");
			...
			>
				...
				>
					{academyAccessRequired ? (
						...
						<ContestCardSkeleton width={stateCardWidth} />
						...
						<Box
							className="rounded-[32px] bg-card px-6 items-center justify-center"
							style={{ height: stateCardWidth * 0.625, width: stateCardWidth }}
						>
							<Text className="text-base font-semibold">
								{`${scopeLabel} ${t("contest.empty.notFoundSuffix", "contest topilmadi")}`}
							</Text>
							<Text className="mt-2 text-center text-sm text-muted-foreground">
								{t(
									"contest.empty.description",
									"Backenddan contestlar kelganda shu yerda ko'rinadi.",
								)}
							</Text>
						</Box>
					)}
				...
				{academyAccessRequired ? null : (
					<RankingPanel
						currentUser={currentRankingUser}
						emptyMessage={rankingEmptyMessage}
						isLoading={rankingQuery.isLoading || rankingQuery.isFetching}
