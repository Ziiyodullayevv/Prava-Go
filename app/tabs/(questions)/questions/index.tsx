...
export default function QuestionsSearchScreen() {
	...
	const toggleSavedQuestion = (question: QuizQuestion) => {
	...
	return (
		<Box className="flex-1 pt-safe bg-background">
			...
			<Box className="px-4 mt-4">
				...
				{isFilterOpen ? (
					<Box
						className="mt-2 rounded-3xl bg-card shadow-hard-5 overflow-hidden"
						style={{ maxHeight: 300 }}
					>
			...
			<FlashList
				...
				keyExtractor={(item, index) =>
					item.type === "skeleton" ? item.id : String(item.item.id ?? index)
				}
				renderItem={({ item }) =>
					item.type === "skeleton" ? (
						<QuestionSkeleton />
						...
						<QuestionCard
							...
							categoryLabel={
								categoryTitleById.get(
									String(item.item.category_id ?? item.item.category ?? ""),
								) ??
							...
							mutedColor={muted}
							textColor={text}
							isSaved={savedQuestionIds.has(String(item.item.id))}
							isSaving={
								savingQuestionIds.has(String(item.item.id))
							}
							onPress={() =>
								router.push({
									pathname: "/tabs/(questions)/questions/[questionId]",
									params: { questionId: String(item.item.id) },
								})
							}
							onToggleSaved={() => toggleSavedQuestion(item.item)}
							questionLabelText={t("practice.questions.detailTitle", "Savol")}
							savedLabel={t("practice.questions.saved", "Saqlangan")}
						/>
					)
				}
