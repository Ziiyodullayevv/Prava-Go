...
function SignCard({ item, onPress }: { item: SignListItem; onPress: () => void }) {
	return (
		<YandexRippleButton onPress={onPress} borderRadius={16}>
			<Box
				className="bg-card rounded-2xl items-center py-3 px-2 shadow-hard-5"
...
export default function SignCategoryDetailScreen() {
	...
	const signsQuery = useSectionSignsQuery(categoryId);
	const signDetailQuery = useSignDetailQuery(selectedSignId, Boolean(selectedSignId));
